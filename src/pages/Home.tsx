...
import { PlayCircle, Users, Trophy, TrendingUp, Activity, Loader2 } from 'lucide-react';
  ...
  return (
    <Layout title="Painel Principal">
        ...
        {cards.map((card) => (
          <button
            ...
            className={`relative flex flex-col text-left bg-white p-6 rounded-2xl shadow-sm border transition-all group active:scale-[0.98]
              ${card.isLive ? 'border-red-200 shadow-red-100 ring-1 ring-red-100' : 'border-slate-100 hover:shadow-md hover:border-slate-200'}
            `}
          >
            ...
            <h3 className={`text-lg font-bold mb-1 group-hover:opacity-80 transition-opacity ${card.isLive ? 'text-red-700' : 'text-slate-800'}`}>
              {card.title}
            </h3>
      ...
      <div className="mt-8 bg-linear-to-r from-slate-900 to-slate-800 rounded-2xl p-6 text-white shadow-lg">
        {hasActiveMatch ? (
          <>
            <h3 className="font-bold text-lg mb-2 flex items-center gap-2">⚠️ Atenção, Admin!</h3>
            <p className="text-slate-300 text-sm mb-4">
              Existe uma partida rolando agora. Não esqueça de registrar os gols em tempo real.
            </p>
            <button 
          ...
          <>
            ...
            <p className="text-slate-300 text-sm mb-4">
              Mantenha as notas dos jogadores atualizadas para garantir times equilibrados hoje.
            </p>
            <button 
