...
export default function History() {
  ...
  return (
    <Layout title="Histórico de Jogos">
      {loading ? (
         <div className="text-center py-10 text-slate-400">Carregando jogos...</div>
      ) : matches.length === 0 ? (
         <p className="text-center py-10 text-slate-400">Nenhuma partida finalizada ainda.</p>
      ) : (
                ...
                {dayMatches.map((match, idx) => (
                  <div key={match.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="bg-slate-50 px-4 py-2 border-b border-slate-100 flex justify-between items-center text-xs text-slate-500">
                       <span className="font-bold">Jogo {dayMatches.length - idx}</span>
                    ...
                    <div className="p-4 flex items-center justify-between">
                       ...
                       <div className="flex-1 text-center">
                          <div className={`text-2xl font-black ${match.scoreRed > match.scoreBlue ? 'text-red-600' : 'text-slate-400'}`}>
                            {match.scoreRed}
                          </div>
                          <div className="text-xs text-red-600 font-bold uppercase mt-1">Vermelho</div>
                       </div>
                       ...
                       {/* Time Blue */}
                       <div className="flex-1 text-center">
                          <div className={`text-2xl font-black ${match.scoreBlue > match.scoreRed ? 'text-blue-600' : 'text-slate-400'}`}>
                            {match.scoreBlue}
                          </div>
                          <div className="text-xs text-blue-600 font-bold uppercase mt-1">Azul</div>
                       </div>
                    </div>

                    {/* Detalhes dos Times (Expansível ou Pequeno) */}
                    <div className="px-4 pb-3 pt-0 text-xs text-slate-400 text-center">
                       <p><span className="font-bold">Elenco V:</span> {match.teamRed.map(id => players.get(id)?.split(' ')[0]).join(', ')}</p>
                       <p className="mt-1"><span className="font-bold">Elenco A:</span> {match.teamBlue.map(id => players.get(id)?.split(' ')[0]).join(', ')}</p>
                    </div>
                  </div>
                ))}
              </div>
